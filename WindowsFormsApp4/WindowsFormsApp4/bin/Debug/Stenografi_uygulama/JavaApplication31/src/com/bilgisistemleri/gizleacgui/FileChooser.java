 ...
 */
package com.bilgisistemleri.gizleacgui;
...
public class FileChooser extends javax.swing.JFrame
{
    ...
    public FileChooser(String encryptOrDecrypt)
    {
        ...
        initComponents();
        jFileChooser1.setDialogTitle("Dosyaları ve dosyalarını almak istediğiniz klasörleri seçin "+encryptOrDecrypt);
        ...
        {
            UIManager.put("FileChooser.openDialogTitleText", "Şifrelenecek Dosyaları Seçin");
            UIManager.put("FileChooser.openButtonText", "Şifrelenecek Dosyaları Seçin");
        }
        else if(encryptOrDecrypt.equalsIgnoreCase("decrypt"))
        {
            UIManager.put("FileChooser.openDialogTitleText", "Şifresini Çözülecek Dosyaları Seçin");
            UIManager.put("FileChooser.openButtonText", "Şifresini Çözülecek Dosyaları Seçin");
        }
    ...
    private boolean anyEncFileExists(File[] listOfFilesAndFolders)
    {
        for(File file:listOfFilesAndFolders)
        {
            ...
            if(!file.isDirectory() && file.getName().startsWith("encrypted")) 
            {
                System.out.println(file.getAbsoluteFile()+ " zaten şifreli");
                Toolkit.getDefaultToolkit().beep();
                JOptionPane.showMessageDialog(null, "1 veya daha fazla dosya zaten şifreli !! \nLütfen tekrar seçim yapın", "hata", JOptionPane.ERROR_MESSAGE);
                return true;
            } 
        }
        return false;
    ...
    {     //!filePath.substring(filePath.length()-4, filePath.length()).equals(".enc")
       for(File file:listOfFilesAndFolders)
        {
            String filePath=file.getAbsoluteFile().toString();
            if(!file.isDirectory() && !file.getName().substring(0, 9).equals("encrypted"))
            {
                System.out.println(file.getAbsoluteFile()+ " şifreli değil");
                Toolkit.getDefaultToolkit().beep();
                JOptionPane.showMessageDialog(null, "1 veya daha fazla dosya şifreli değil !! \nLütfen tekrar seçim yapın","hata", JOptionPane.ERROR_MESSAGE);
                return false;
            } 
        }
        return true;
